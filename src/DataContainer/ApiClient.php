...
namespace Craffft\ApiClientBundle\DataContainer;
...
class ApiClient extends Backend
{
    ...
    public function storeDateAdded($dc)
    {
        ...
        $this->Database->prepare("UPDATE tl_api_client SET dateAdded=? WHERE id=?")
            ->execute($time, $dc->id);
    }
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        // Update the database
        $this->Database->prepare("UPDATE tl_api_client SET tstamp=$time, disable='" . ($blnVisible ? '' : 1) . "' WHERE id=?")
            ->execute($intId);

        $objVersions->create();
        $this->log('A new version of record "tl_api_client.id=' . $intId . '" has been created' . $this->getParentEntries('tl_api_client',
     ...
     * @return string
     */
    public function setDefaultClientId($varValue, DataContainer $dc)
    {
        if (empty($varValue)) {
